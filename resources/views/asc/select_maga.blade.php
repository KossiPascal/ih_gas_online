        ...
        <h1>{{__('messages.Selectionner stock vente')}}</h1>
            <table id="liste_produit" class="display table table-striped table-bordered data-table">
                ...
                @foreach($magasins as $magasin)
                    <tr>
                        <td>{{$magasin->mag_lib}}</td>
                        <td style="text-align: left">{{($magasin->mag_type)}}</td>
                        <td><a href="{{route('vente.mag_source',$magasin->mag_num)}}" class="btn btn-danger">{{__('messages.Selectionner')}}</a></td>
                    </tr>
                @endforeach
                <?php $count++;?>
                </tbody>
