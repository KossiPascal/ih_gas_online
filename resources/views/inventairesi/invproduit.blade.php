...
@section('content')
    <main class="col-sm-12 ml-sm-auto col-md-12 pt-0" style="text-decoration: none; margin-top: 5px;">
        <div class="col-12 col-sm-12 col-md-12">
            <h5 class="ml-5">{{__('messages.FICHE DE STOCK')}}</h5>
            ...
            <div class="info-box">
                <div class="table-responsive">
                    <table class="table table-striped table-bordered" id="fiche_produit">
                        <thead>
                        <tr class="cart_menu">
                            <td class="price">{{__('messages.Date')}}</td>
                            <td class="price">{{__('messages.Libelle')}}</td>
    ...
    <script>
        function produit() {
            $.ajax({
                ...
                {
                    $('#produit_id').empty();
                    $('#produit_id').append('<option id="0"  value="0">- {{__('messages.Choisir un produit')}} -</option>');
                    for (var i = 0; i < data.length; i++) {
                        $('#produit_id').append('<option id=' + data[i].produit_id + ' value=' + data[i].produit_id + '>'+ data[i].nom_commercial +'</option>');
                    }
                    $('#produit_id').change();
                }
            })
        ...
        $(document).ready(function(){
            ...
            $('#imprimer').click(function(){
                ...
                var the_url = "invsi.print_invproduit/"+debut+"/"+fin+"/"+produit_id;
                $.ajax({
