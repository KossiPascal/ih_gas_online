    ...
    <main class="col-sm-12 ml-sm-auto col-md-12 pt-0" style="text-decoration: none; margin-top: 5px;margin-bottom: 20px">
        ...
        <div class="col-12 col-sm-12 col-md-12">
            ...
            <div class="col-12 col-sm-6 col-md-4 float-left">
                <a href="{{route('recsi.histo')}}" class="btn btn-danger"><i class="fa fa-list"></i>{{__('messages.Historique des receptions')}}</a>
            </div>
            <div class="col-12 col-md-4 float-left">
                <a href="{{route('recsi.index')}}" class="btn btn-success"><i class="fa fa-plus"></i> {{__('messages.Suivi Commande')}}</a>
            </div>
            <div class="col-12 col-sm-6 col-md-4 float-right">
                <a href="{{route('trsi.index')}}" class="btn btn-warning"><i class="fa fa-check"></i> {{__('messages.Transfert vers FS')}}</a>
            </div>
        </div>
...
@section('extra-js')
    <script>
        ...
        function actualiser() {
            ...
            if (commande_id!=0){
                $('#pdt_cmde').load('recsi.pdt_cmde/'+commande_id);
                $('#pdt_rec').load('recsi.pdt_rec/'+code+'/'+commande_id);
        ...
        $(document).ready(function(){
                ...
                $.ajax({
                    ...
                    success:function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#pdt_rec').load('recsi.pdtrec/'+code+'/'+commande_id);
            ...
            $(document).on('click', '.editer', function(){
                ...
                $('#form_result').html('');
                $.ajax({
                    url:"recsi.select_edit/"+id+"/"+commande_id,
                    dataType:"json",
                    success:function(data){
                        ...
                        $('#produit_reception_si_id').val(data.produit.produit_reception_si_id);
            ...
            $(document).on('click', '.select', function(){
                ...
                $('#form_result').html('');
                $.ajax({
                    url:"recsi.select/"+id+"/"+commande_id,
                    dataType:"json",
                    success:function(data){
