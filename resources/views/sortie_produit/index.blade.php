...
@section('content')
    <main class="col-sm-12 ml-sm-auto col-md-12 pt-0" style="text-decoration: none margin-top: 5px">
        ...
        <div class="info-box mt-5">
            <x-datatable id="list-sorties" url="{{ route('ajax.sortie-produits.index') }}" :columns="json_encode([
                ['data' => 'sortie_produit_id', 'name' => 'sortie_produit_id'],
                ...
                <x-slot:head>
                    <tr>
                        ...
                        <th>{{ trans('messages.Date Entree') }}</th>
                    </tr>
                </x-slot:head>
            </x-datatable>

            <x-modals.form id="profil-form" create-id="create-product-entry" list-id="list-sorties"
                url="{{ route('ajax.sortie-produits.store') }}">
                <x-slot:title>
                    {{ trans('messages.nouvel_sortie_produit') }}
                </x-slot>

                <x-form.fields.select name="magasin_id" label="Magasin" required>
                    <option value="">{{ trans('messages.selectionnez magasin') }}</option>
            ...
            <x-modals.delete id="delete-profil-data" list-id="list-sorties"
                url="{{ route('ajax.sortie-produits.index') }}" />
        </div>
    </main>
@endsection
...
<tr>
    ...
    <td>
        <div class="form-group">
            <input type="number" name="produits[${i}][qte]" class="form-control" min="1" required placeholder="{{ trans('messages.Qte recue') }}">
        </div>
    </td>
    <td>
        <div class="form-group">
