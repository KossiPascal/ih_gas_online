...
@section('content')
    <main class="col-sm-12 ml-sm-auto col-md-12 pt-0" style="text-decoration: none; margin-top: 5px;margin-bottom: 20px">
        <div class="col-md-12"><h3 class="ml-5" style="text-align:center">{{__('messages.EDITER APPROVISIONNEMENT')}}</h3></div>
        <div class="col-md-12">
            ...
            <div class="col-md-6 float-right" style="text-align: right">
                <a href="{{route('appromag.histo')}}" class="btn btn-warning"><i class="fa fa-list"></i> {{__('messages.HISTORIQUE DES APPROVISIONNEMENT')}}</a>
            </div>
        </div>
                        ...
                        <table class="table table-striped table-bordered contour_table" id="pdt_selected">
                            ...
                            @foreach($pdtcon as $produit)
                                <tr>
                                    ...
                                    <td class="cart_price">{{$produit->pdt_pa}}</td>
                                    <td class="cart_price">{{$produit->qte}}</td>
                                    <td class="cart_total">{{$produit->mont}}</td>
                                    <td class="cart_delete">
                                        <a href="{{route('appromag.delete',[$produit->am_num,$produit->pdt_num])}}" class="btn btn-danger"><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                            <?php $count++;?>
                        ...
                        <table id="liste_produit" class="display table table-striped table-bordered data-table">
                            ...
                            @foreach($produits as $produit)
                                <tr>
                                    ...
                                    <td width="6" style="text-align: right">{{getPrice3($produit->pdt_pa)}}</td>
                                ...
                                <div class="modal-body">
                                    <form method="post" id="clt_form" class="form-horizontal">
                                        ...
                                        <div class="form-group" align="center">
                                            ...
                                            <input type="hidden" name="hidden_am_num" id="hidden_am_num" />
    ...
    <script>
        $(document).ready(function(){
            ...
            $(document).on('click', '.selectedit', function(){
                var id = $(this).attr('id');
                var am_num = document.getElementById('am_num').value;
                document.getElementById('am_num').value = am_num;
                $('#form_result').html('');
                $.ajax({
                    ...
                    success:function(html){
                        ...
                        $('#hidden_am_num').val(am_num);
                        $('.modal-title').text("{{__('messages.Saisir la quantite recu')}}");
            ...
            $(document).on('click', '.editer', function(){
                ...
                $.ajax({
                    ...
                    success:function(html){
                        ...
                        $('#hidden_id').val(html.data.pdt_num);
                        $('.modal-title').text("{{__('messages.Modifier la quantiter a commander')}}");
                        $('#action_button').val("{{__('messages.Modifier')}}");
                        $('#action').val("{{__('messages.Modifier')}}");
                        $('#clientModal').modal('show');
                    }
                })
            });
