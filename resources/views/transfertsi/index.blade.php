    ...
    <main class="col-sm-12 ml-sm-auto col-md-12 pt-0" style="text-decoration: none; margin-top: 5px;margin-bottom: 20px">
        ...
        <div class="col-12 col-sm-12 col-md-12">
            ...
            <div class="col-12 col-sm-6 col-md-4 float-left">
                <a href="{{route('trsi.histo')}}" class="btn btn-danger"><i class="fa fa-list"></i>{{__('messages.Historique des transerts')}}</a>
            </div>
                                ...
                                <div class="modal-body">
                                    ...
                                    <form method="post" id="add_form" class="form-horizontal">
                                        ...
                                        <div class="form-group" align="center">
                                            ...
                                            <input type="hidden" name="produit_reception_si_id" id="produit_reception_si_id" />
                                            <input type="hidden" name="hidden_code" id="hidden_code" />
                                            <input type="hidden" name="hidden_reception_si_id" id="hidden_reception_si_id" />
    ...
    <script>
        function reception_si() {
            $.ajax({
                ...
                {
                    $('#reception_si_id').empty();
                    $('#reception_si_id').append('<option id="0"  value="0">- {{__('messages.Choisir une reception')}} -</option>');
                    for (var i = 0; i < data.length; i++) {
                        $('#reception_si_id').append('<option id=' + data[i].reception_si_id + ' value=' + data[i].reception_si_id + '>'+ data[i].code +' - ' + data[i].nom_centre +'</option>');
                    }
                    $('#reception_si_id').change();
                }
            })
        }
        ...
        function actualiser() {
            ...
            reception_si_id = document.getElementById("reception_si_id").value;
            if (reception_si_id!=0){
                $('#pdt_tr').load('trsi.pdt_tr/'+reception_si_id);
                $("div.dataTables_filter input").focus();
                document.getElementById("hidden_reception_si_id").value=reception_si_id;
                getcommande(reception_si_id);
            }
        }
        function getcommande(id){
            $.ajax({
                url:"trsi.getcommande/"+id,
                dataType:"json",
                success:function(data)
                {
        ...
        $(document).ready(function(){
            ...
            $('#add_form').on('submit', function(event){
                ...
                var reception_si_id = document.getElementById('reception_si_id').value;
                $.ajax({
                    url:"{{ route('trsi.add') }}",
                    method:"POST",
            ...
            $(document).on('click', '.select', function(){
                ...
                $.ajax({
                    ...
                    success:function(data){
                        ...
                        $('#produit_reception_si_id').val(id);
            ...
            $(document).on('click', '.delete', function(){
                ...
                $.ajax({
                    ...
                    success:function(data)
                    {
                        ...
                        $('#pdt_rec').load('trsi.pdtrec/'+code+'/'+reception_si_id);
                        $("div.dataTables_filter input").focus();
                    }
                })
            });
