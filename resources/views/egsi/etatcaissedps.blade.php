    ...
    <main class="col-sm-12 ml-sm-auto col-md-12 pt-0" style="text-decoration: none; margin-top: 5px;">
        <div class="col-md-12">
            <div class="col-md-4 float-left">
                <h4 class="ml-5">{{__('messages.VENTE PAR DPS')}}</h4>
            </div>
    ...
    <script>
        function direction() {
            $.ajax({
                ...
                {
                    $('#dps_id').empty();
                    $('#dps_id').append('<option id="0"  value="0">- {{__('messages.Choisir une direction')}} -</option>');
        ...
        $(document).ready(function(){
            ...
            function load_data(from_date = '', to_date = '', dps_id = '')
            {
                $('#histo_prod').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                        url:'{{ route("eg.etatcaissedps") }}',
                        data:{from_date:from_date, to_date:to_date, dps_id:dps_id}
                    },
                    columns: [
                        {
                            data:'libelle',
            ...
            $('#filter').click(function(){
                ...
                console.log(from_date, to_date, dps_id)
                if(from_date != '' && to_date != '' && dps_id != ''){
                    $('#histo_prod').DataTable().destroy();
                    load_data(from_date, to_date,dps_id);
                }
                else
                {
            ...
            $('#imprimer').click(function(){
                ...
                var the_url = "eg.print_efdps/"+debut+"/"+fin+"/"+dps_id;
                $.ajax({
