...
use App\Models\ProduitTransfertSi;
use App\Models\TransfertSi;
...
class TransfertSiController extends Controller
{
    protected function code(){
        $debut = date('Y').'-'.date('m').'-01';
        $fin = date('Y-m-d');
        $achatp = DB::table('transfert_sis')
    ...
    {
        $transfert = new TransfertSi();
        $code = $this->code();
        $reception_sis = [];
        Session::forget('code');
        return view('transfertsi.index', compact('transfert','code','reception_sis'));
    }

    public function reception(){
        if (\request()->ajax()){
            $reception_sis = DB::table('reception_sis')
                ->join('centres','centres.centre_id','=','reception_sis.centre_id')
                //->join('commandes','commandes.commande_id','=','reception_sis.commande_id')
                ->whereIn('reception_sis.etat_liv', ['LivreeSI','PartielleSI'])
                ->get();
            return $reception_sis;
        }
    }

    public function getcommande($id){
        $pdtcon = DB::table('reception_sis')
            ->where('reception_si_id', '=', $id)
            ->get();
    ...
    {
        $pdtcon = DB::table('produit_reception_sis')
            ->where('reception_si_id','=',$prsi_id)
            ->get();
        $output='<table class="table table-striped table-bordered contour_table" id="pdt_tr">
           ...
           <tbody>';
            foreach($pdtcon as $produit){
                $button_edit = '<button type="button" name="editer" id="'.$produit->produit_reception_si_id.'" class="select btn btn-success btn-sm"><i class="fa fa-edit"></i></button>';;
                //$button_supp = '<button type="button" name="delete" id="'.$produit->produit_reception_si_id.'" class="select btn btn-danger btn-sm"><i class="fa fa-trash"></i></button>';;
    ...
    public function select($id){
        ///if(request()->ajax()) {
            $produit = ProduitReceptionSi::find($id);
            $qte_liv = $pdtcon = DB::table('produit_reception_sis')
                ->where('produit_reception_si_id','=',$id)
                ->sum('qte_recue');
    ...
    public function add(Request $request){
        ...
        try {
            if ($request->qte_cmde>=($request->qte_tr+$request->qte_liv)){
                ProduitReceptionSi::find($request->produit_reception_si_id)->update([
                    'qte_transferee' =>$request->qte_tr,
                    'remarque' =>$request->remarque
                ]);
                DB::connection('vps')->table('produit_reception_sis')->where('produit_reception_si_id',$request->produit_reception_si_id)->update([
                    'qte_transferee' =>$request->qte_tr,
                    'remarque' =>$request->remarque
                ]);
                DB::commit();
    ...
    /*public function delete($id){
        if(request()->ajax()) {
            ProduittransfertSi::find($id)->delete();
        }
    }*/
    ...
    public function store(Request $request){
        ...
        $form_data = array(
            ...
            'reception_si_id' =>  $request->reception_si_id,
        ...
        try {
            ...
            TransfertSi::create($form_data);
            DB::connection('vps')->table('transfert_sis')->insert($form_data);
            DB::table('reception_sis')
                ->where('reception_si_id','=',$request->reception_si_id)
                ->update(['etat_liv'=>'Transferee']);
            DB::connection('vps')->table('reception_sis')
                ->where('reception_si_id','=',$request->reception_si_id)
                ->update(['etat_liv'=>'Transferee']);

            Alert::success('Success !', 'transfert enregistre avec success.');
            DB::commit();
            return redirect()->route('trsi.index');
        }catch (\PDOException $se){
            DB::rollBack();
            dd($se);
    ...
    {
        //Session::put('transfert_si_id',$id);
        //return redirect()->route('rec.editer');
    }

    public function editer(){
        /*$transfert_si_id = Session::get('transfert_si_id');

        if (Session::get('transfert_si_id')){
            $transfert = transfertSi::find($transfert_si_id);
            $code = $transfert->code;
            $reception_sis = DB::table('reception_sis')
                ->where('reception_si_id','=',$transfert->reception_si_id)
                ->get();
            if (Auth::user()->ut==1){
                return view('transfert_si.edit', compact('transfert','code','magasins','reception_sis'));
            }elseif (Auth::user()->ut=2){
                return view('transfert_si.editc', compact('transfert','code','magasins','reception_sis'));
            }elseif (Auth::user()->ut==3){
                return view('transfert_si.editp', compact('transfert','code','magasins','reception_sis'));
            }else{
                //Rien a faire
            }
        }else{
    ...
    public function histo(Request $request){
        ...
        {
            $historiques = DB::table('transfert_sis')
                ->join('centres','centres.centre_id','=','transfert_sis.centre_id')
                ->join('reception_sis','reception_sis.reception_si_id','=','transfert_sis.reception_si_id')
                ->join('commandes','commandes.commande_id','=','reception_sis.commande_id')
                ->whereBetween('transfert_sis.date_transfert', array($request->from_date, $request->to_date))
                ->get();
        }
        else
        {
            ...
            $historiques = DB::table('transfert_sis')
                ...
                ->join('reception_sis','reception_sis.reception_si_id','=','transfert_sis.reception_si_id')
                ->join('commandes','commandes.commande_id','=','reception_sis.commande_id')
                ->whereBetween('transfert_sis.date_transfert', array($debut, date('Y-m-d')))
                ->get();
        }
        ...
        return view('transfertsi.histo', compact('historiques'));
    }


    protected function show($id){
        $transfert = DB::table('transfert_sis')
            ->join('users','users.id','=','transfert_sis.user_id')
            ->join('reception_sis','reception_sis.reception_si_id','=','transfert_sis.reception_si_id')
            ->join('commandes','commandes.commande_id','=','reception_sis.commande_id')
            ->where('transfert_sis.transfert_si_id','=', $id)
            ->get();
        ...
        if (count($transfert)==0){
            ...
            $categories = DB::table('produit_reception_sis')
                ->join('produits','produits.produit_id','=','produit_reception_sis.produit_id')
                ->join('categories','categories.categorie_id','=','produits.categorie_id')
                ->join('reception_sis','reception_sis.reception_si_id','=','produit_reception_sis.reception_si_id')
                ->join('transfert_sis','transfert_sis.reception_si_id','=','reception_sis.reception_si_id')
                ->where('transfert_sis.transfert_si_id','=',$id)
            ...
            foreach($categories as $categorie){
                $produits = DB::table('produit_reception_sis')
                    ->join('produits','produits.produit_id','=','produit_reception_sis.produit_id')
                    ->join('reception_sis','reception_sis.reception_si_id','=','produit_reception_sis.reception_si_id')
                    ->join('transfert_sis','transfert_sis.reception_si_id','=','reception_sis.reception_si_id')
                    ->where('transfert_sis.transfert_si_id','=',$id)
    ...
    protected function rec_cmde($id){
        $commande = DB::table('commande')
            ...
            ->where('commande.reception_si_id','=', $id)
            ->get();
        if (count($commande)==0){
            ...
            $transferts = DB::table('transfert_sis')
                ->join('magasins','magasins.magasin_id','=','transfert_sis.magasin_id')
                ->join('users','users.id','=','transfert_sis.user_id')
                ->where('transfert_sis.reception_si_id','=', $commande->reception_si_id)
                ->get();
            ...
            <table class="table-bordered" style="width: 100%; border: 1px solid; border-color: #000000; border-radius: 10px">
                <tr>
                    <td colspan="2">LES LIVRAISONS DE LA commande N° <b>' .$commande->reception_si_id.' DU ' .$cmde_date.' ADRESSEE A '.$commande->nom.'</b></td>
                </tr>
            </table>
            <br>
            ...
            foreach($transferts as $transfert){
                $produits = DB::table('produit_reception_sis')
                    ->join('produits','produits.produit_id','=','produit_reception_sis.produit_id')
                    ->where('produit_reception_sis.code','=',$transfert->code)
                    ->get();
